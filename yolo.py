...
class YOLO(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        with torch.no_grad():
            ...
            top_boxes   = xywhr2xyxyxyxy(top_boxes)
        # 设置字体与边框厚度
        ...
        for i, c in list(enumerate(top_label)):
            ...
            draw.polygon(box, outline=self.colors[c])
    ...
    def detect_heatmap(self, image, heatmap_save_path):
        ...
        with torch.no_grad():
            ...
            if self.cuda:
                images = images.cuda()
            # 将图像输入网络当中进行预测！
            dbox, cls, x, angle, anchors, strides = self.net(images)
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            f.write("%s %s %s %s %s %s %s\n" % (predicted_class, score[:7], str(int(x)), str(int(y)), str(int(w)),str(int(h)),str(int(angle))))
