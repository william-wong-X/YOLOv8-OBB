...
if __name__ == '__main__':
    ...
    #--------------------------------------------------------------------------------------#
    classes_path    = 'model_data/bridge_classes.txt'
    #--------------------------------------------------------------------------------------#
    ...
    #   默认指向根目录下的VOC数据集
    #-------------------------------------------------------#
    DOTA_path  = 'GLH-Bridge-split'
    #-------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    #-------------------------------------------------------#
    map_out_path    = 'map_out'

    image_ids = open(os.path.join(DOTA_path, "ImageSets/Main/test.txt")).read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path  = os.path.join(DOTA_path, "images/val/"+image_id+".jpg")
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                with open(os.path.join(DOTA_path, "labels/val/"+image_id+".txt"), 'r') as file:
                    for line in file:
                        box = line.strip().split()
                        box = np.array(box, dtype=int)
                        box_new = xyxyxyxy2xywhr(np.expand_dims(box[1:], 0))
                        box = np.concatenate([[box[0]], box_new.squeeze()])
                        obj_name = class_names[int(box[0])]
                        x, y, w, h, angle = box[1:]
                        
                        new_f.write("%s %s %s %s %s %s\n" % (obj_name, str(x), str(y), str(w), str(h), str(angle)))
